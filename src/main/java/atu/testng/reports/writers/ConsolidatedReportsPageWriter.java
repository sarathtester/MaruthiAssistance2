	...
	public static void menuLink(PrintWriter paramPrintWriter, int paramInt) {
		...
		if (paramInt == 1)
			...
			for (int i = 1; i <= paramInt; ++i) {
				...
				if (j == paramInt) {
					paramPrintWriter.println(
							"\n <li style=\"padding-top: 4px;padding-bottom: 4px;\"><a href=\"" + Directory.RUNName + (j-i+1)
									+ Directory.SEP + "CurrentRun.html\" >" + "Run " + (j-i+1) + " </a></li>\n");
					break;
				}
				paramPrintWriter.println("\n <li class=\"menuStyle\"><a href=\"" + Directory.RUNName + (j-i+1) + Directory.SEP
						+ "CurrentRun.html\" >" + "Run " + (j-i+1) + " </a></li>\n");
				}
			}
		paramPrintWriter.println("\n                    </ul>\n                </td>\n\n");
	}
